...
class Incidence(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    creation_user_id = db.Column(db.Integer, db.ForeignKey('users.id', name='fk_creation_user_id'), nullable=False)
...
class Like(db.Model):
    __tablename__ = 'likes'
    userid = db.Column(db.Integer, db.ForeignKey('users.id', name='fk_like_user_id'), primary_key=True)
    incidenceid = db.Column(db.Integer, db.ForeignKey('incidences.id', name='fk_like_incidence_id'), primary_key=True)
    timestamp = db.Column(db.DateTime, default=db.func.current_timestamp(), nullable=False)
...
class Comment(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    userid = db.Column(db.Integer, db.ForeignKey('users.id', name='fk_comment_user_id'), nullable=False)
    incidenceid = db.Column(db.Integer, db.ForeignKey('incidences.id', name='fk_comment_incidence_id'), nullable=False)
    timestamp = db.Column(db.DateTime, default=db.func.current_timestamp(), nullable=False)
    text = db.Column(db.Text)
    removed = db.Column(db.DateTime)
    removed_by_id = db.Column(db.Integer, db.ForeignKey('users.id', name='fk_comment_removed_by_id'))
    user = db.relationship('User', foreign_keys=[userid],  backref=db.backref('comments', lazy=True))
    removed_by = db.relationship('User', foreign_keys=[removed_by_id], backref=db.backref('removed_comments', lazy=True))
